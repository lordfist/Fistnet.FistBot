...
{
    public class TestEnemyHeadingWithin90 : TestBase
    {
        public override TestTypeEnum TestType
        {
            get { return TestTypeEnum.TestEnemyHeadingWithin90; }
        }
        ...
        public override bool Evaluate()
        {
                ...
                && (this.Owner.Time - this.Owner.ActiveField.TrackedEnemy.Time) < Common.MAX_TIME_DISTANCE_TEST)
            {
                if (this.Owner.ActiveField.TrackedEnemy.Heading.HasValue)
                    eval = (Math.Abs(this.Owner.ActiveField.TrackedEnemy.Heading.Value - this.Owner.ActiveField.TurnStatus.Status.Heading) <= 90);
            }

            return eval;
        }

        public TestEnemyHeadingWithin90(BotBase owner)
            : base(owner)
        {
        }
    }
